...
This applies to TSM/META research and later authorized evaluations of other tools. The goal is to learn the complete operating contract needed by our own implementation, not to copy another product.
...
1. Create a capture session with `scripts/New-CaptureSession.ps1`.
...
7. Promote approved metadata into the future runtime registry consumed by the installer UI.
...
sessions/                sanitized capture-session manifests
...
Authorized test session
        ...
        -> private artifact store / vendor source
        -> Software Builder installer plan
        -> signed THETECHGUY installer
```

The installer must verify tool ID, channel, version, source, file name, size, architecture and SHA-256 before executing or installing anything. Missing or conflicting evidence fails closed.
